...
let add_literal (objects: objects) (facts: local_facts)
...
let collect_pointsto_step (globals_are_properties: bool) (objects: objects)
    (state: points_to_map) facts: clean_operation -> points_to_map = function
    | CFunPre { args } ->
    ...
    | CRead { name; value; isGlobal } ->
        let ref =
            reference_of_variable globals_are_properties facts isGlobal name in
