...
let pp_print_stack pp = let open Format in
  List.iter (function
...
let pp_cond pp cond = Format.pp_print_string pp (match cond with
...
let pp_path pp = function
  | [] ->
    Format.pp_print_string pp "(top)"
  | p::l ->
    Format.pp_print_string pp p;
    List.iter (fun p -> Format.pp_print_char pp '.'; Format.pp_print_string pp p) l
     
let pp_obj_match_failure pp = let open Format in function
...
let pp_fun_match_failure pp = let open Format in function
...
let pp_reason pp = let open Format in let str = pp_print_string pp in function
...
let pp_failed pp (failed_cons, op) =
  Format.fprintf pp "@[<v 2>%a failed because the following conditions don't hold:@ %a@]@ "
    pp_match_operation op
    (FormatHelper.pp_print_list_lines
      (fun pp (cond, reason) -> Format.fprintf pp "%a because %a" pp_cond cond pp_reason reason))
      failed_cons
