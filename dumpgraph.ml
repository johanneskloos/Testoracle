...
let dump_cert path =
	...
	printf "@[<v>";
	TraceTree.iter_vertex (fun v -> match TraceNodes.find v nodes with
	| FinalNodeData { op1; op2; stack; trace_trace } ->
		printf "@[<hov 2>Node %d: final! %a vs. %a, stack %a, failed: @[<v>@ %a@]@]@ "
			...
			(FormatHelper.pp_print_list_lines pp_failed) trace_trace
	| NodeData { op1; op2; stack; trace_trace } ->
		printf "@[<hov 2>Node %d: %a vs. %a, stack %a, failed: @[<v>@ %a@]@]@ "
			...
			(FormatHelper.pp_print_list_lines pp_failed) trace_trace
	| EndtraceData ops ->
		printf "@[<v 2>Node %d: left-over trace:@ %a@]@ "
