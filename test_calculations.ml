...
let check_parameters_equals got exp =
    check_int_opt_list "callee parameters"
      (List.map (fun x -> x.last_parameters) got)
      exp.params
...
let test_parameters = test_one check_parameters_equals
...
let make_tests_for path =
    [make_one_test "caller arguments" test_arguments path;
     make_one_test "callee parameters" test_parameters path;
