...
|]
let functions_call = [|
  Local { instrumented = "f ver. 1"; uninstrumented = "f ver. 1" };
  Local { instrumented = "f ver. 2"; uninstrumented = "f ver. 2" };
|]
...
let facts_call_5 = { facts_call_4 with last_parameters = Some 3 }
...
let compare_step
  (rt1, {last_arguments=la1; last_parameters=lp1; last_update=lu1; versions=ver1; aliases=al1})
  (rt2, {last_arguments=la2; last_parameters=lp2; last_update=lu2; versions=ver2; aliases=al2}) =
